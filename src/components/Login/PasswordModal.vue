<template>
  <div class="fixed inset-0 bg-gray-600 bg-opacity-75 flex justify-center items-center z-50">
    <div class="bg-white p-6 rounded-lg shadow-lg w-[650px] relative">
      <button @click="$emit('close')" class="absolute top-4 right-4 text-2xl font-bold text-gray-400 hover:text-gray-600">✕</button>
      ...
      <form @submit.prevent="handleSubmit">
        <!-- 이름 입력 -->
        <div class="mb-5">
          <label for="name" class="block text-left text-gray-600 mb-1">이름</label>
        ...
        <!-- 전화번호 입력 -->
        <div class="mb-5">
          <label for="tel" class="block text-left text-gray-600 mb-1">전화번호</label>
          <input type="text" id="tel" v-model="tel" placeholder="전화번호를 입력해주세요 (01012341234)" 
                 class="pl-4 h-[45px] w-full rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-kb-brown-1"/>
          <!-- 전화번호 유효성 검사 경고 메시지 -->
          <p v-if="telError" class="text-red-500 text-sm mt-1">전화번호는 숫자만 10~11자로 입력해주세요.</p>
        </div>
        
        <!-- 생년월일 입력 -->
        <div class="mb-8">
          <label for="birthday" class="block text-left text-gray-600 mb-1">생년월일</label>
          <input type="text" id="birthday" v-model="birthday" placeholder="YYYYMMDD 형식으로 입력해주세요" 
                 class="pl-4 h-[45px] w-full rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-kb-brown-1"/>
          <!-- 생년월일 유효성 검사 경고 메시지 -->
          <p v-if="birthdayError" class="text-red-500 text-sm mt-1">생년월일을 YYYYMMDD 형식으로 입력해주세요.</p>
        </div>
