      ...
      <form @submit.prevent="submitForm">
        <div class="mb-[50px] flex items-center">
          ...
          <input type="text" id="name" v-model="name" placeholder="   이름을 입력해주세요" 
                 class="h-[50px] w-[400px] text-font-color rounded-[15px] border border-kb-brown-1 focus:outline-none focus:ring-2 focus:ring-kb-brown-2"/>
        </div>
        <div class="mb-[50px] flex items-center">
        ...
        <div class="flex items-center">
          ...
          <input type="text" id="email" v-model="email" placeholder="   이메일을 입력해주세요"
                 class="h-[50px] w-[400px] text-font-color rounded-[15px] border border-kb-brown-1 focus:outline-none focus:ring-2 focus:ring-kb-brown-2 mr-[50px]"/>
          <button class="text-lg text-white bg-kb-brown-2 h-[50px] w-[150px] rounded-[15px]">이메일 인증</button><br>
        </div>
        <p class="text-[12px] flex items-center ml-[130px] p-2">반드시 실제로 사용하는 이메일 주소를 입력해주세요</p>
        ...
        <p class="text-[12px] flex items-center ml-[130px] p-2">특수문자(~!#*) 및 영문과 숫자를 포함하여 8~12자로 설정해주세요.</p>
        ...
        <div class="mt-[50px] flex items-center">
          ...
          <input type="text" id="tel" v-model="tel" placeholder="   - 없이 전화번호를 입력해주세요(01012341234)"
                 class="h-[50px] w-[400px] text-font-color rounded-[15px] border border-kb-brown-1 focus:outline-none focus:ring-2 focus:ring-kb-brown-2 mr-[50px]"/>
          <button class="text-lg text-white bg-kb-brown-2 h-[50px] w-[150px] rounded-[15px]">인증번호 발송</button>
        </div>
        <div class="mt-[50px] flex items-center">
          <label for="asset" class="w-[120px]">현재 자산 </label>
        ...
        <div class="flex justify-center mt-[90px] mb-[90px]">
          <button @click="$router.push('/login')"
                  class="w-[213px] h-[50px] text-lg rounded-[15px] mr-10 bg-white text-font-color border border-kb-brown-2">취소</button>
          <button :class="[isFormValid ? 'bg-kb-brown-2' : 'bg-kb-gray-2']" 
                  :disabled="!isFormValid" 
                  @click="submitForm"
                  class="w-[213px] h-[50px] text-lg rounded-[15px] text-font-color">가입하기</button>
        </div>
      </form>
    </div>
  </div>
